...
class MinHash(_Distance):
    ...
    def sim(self, src: str, tar: str) -> float:
        ...
        masks = [
            int(_)
            for _ in np.random.RandomState(seed=self._seed).randint(
                _MININT, _MAXINT, k, dtype=np.int64
            )
        ]

        hashes_src = [_MAXINT] * k
        hashes_tar = [_MAXINT] * k

        for tok in src_tokens:
            hashed_tok = int(sha512(tok.encode()).hexdigest(), 16)
            hashes_src = [
                min(current_hash, mask ^ hashed_tok)
                for current_hash, mask in zip(hashes_src, masks)
            ]
        ...
        for tok in tar_tokens:
            ...
            hashes_tar = [
                min(current_hash, mask ^ hashed_tok)
                for current_hash, mask in zip(hashes_tar, masks)
            ]
        ...
        return cast(
            ...
            sum(
                src_hash == tar_hash
                for src_hash, tar_hash in zip(hashes_src, hashes_tar)
            )
            / k,
        )
