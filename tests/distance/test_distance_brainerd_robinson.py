...
def test_brainerd_robinson_sim():
...
def test_brainerd_robinson_dist():
...
def test_brainerd_robinson_sim_score():
    """Test abydos.distance.BrainerdRobinson.sim_score."""
    # Base cases
    assert cmp.sim_score('', '') == 200.0
    assert cmp.sim_score('a', '') == 100.0
    assert cmp.sim_score('', 'a') == 100.0
    assert cmp.sim_score('abc', '') == 100.0
    assert cmp.sim_score('', 'abc') == 100.0
    assert cmp.sim_score('abc', 'abc') == 200.0
    assert cmp.sim_score('abcd', 'efgh') == 0.0

    assert cmp.sim_score('Nigel', 'Niall') == pytest.approx(abs=1e-7, expected=100.0)
    assert cmp.sim_score('Niall', 'Nigel') == pytest.approx(abs=1e-7, expected=100.0)
    assert cmp.sim_score('Colin', 'Coiln') == pytest.approx(abs=1e-7, expected=100.0)
    assert cmp.sim_score('Coiln', 'Colin') == pytest.approx(abs=1e-7, expected=100.0)
    assert cmp.sim_score('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=127.2727272727)
