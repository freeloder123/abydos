...
def test_johnson_sim_score():
    """Test abydos.distance.Johnson.sim_score."""
    # Base cases
    assert cmp.sim_score('', '') == 2.0
    assert cmp.sim_score('a', '') == 0.0
    assert cmp.sim_score('', 'a') == 0.0
    assert cmp.sim_score('abc', '') == 0.0
    assert cmp.sim_score('', 'abc') == 0.0
    assert cmp.sim_score('abc', 'abc') == 2.0
    assert cmp.sim_score('abcd', 'efgh') == 0.0

    assert cmp.sim_score('Nigel', 'Niall') == pytest.approx(abs=1e-7, expected=1.0)
    assert cmp.sim_score('Niall', 'Nigel') == pytest.approx(abs=1e-7, expected=1.0)
    assert cmp.sim_score('Colin', 'Coiln') == pytest.approx(abs=1e-7, expected=1.0)
    assert cmp.sim_score('Coiln', 'Colin') == pytest.approx(abs=1e-7, expected=1.0)
    assert cmp.sim_score('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=1.3363636364)
