...
"""abydos.tests.distance.test_distance_dice_asymmetric_ii.

This module contains unit tests for abydos.distance.DiceAsymmetricII
"""


import pytest

from abydos.distance import DiceAsymmetricII


cmp = DiceAsymmetricII()


def test_dice_asymmetric_ii_sim():
    """Test abydos.distance.DiceAsymmetricII.sim."""
    # Base cases
    assert cmp.sim('', '') == 1.0
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.7)

def test_dice_asymmetric_ii_dist():
    """Test abydos.distance.DiceAsymmetricII.dist."""
    # Base cases
    assert cmp.dist('', '') == 0.0
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.3)
