...
def test_goodman_kruskal_tau_b_sim():
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.4608002285)
    ...
    assert cmp_no_d.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.2187412587)

    assert GoodmanKruskalTauB(
            intersection_type='linkage',
            alphabet=64,
            tokenizer=QGrams(qval=range(2, 4), skip=1),
        ).sim('adhering', 'gilled') == pytest.approx(abs=1e-7, expected=0.10431548499366636)
    assert GoodmanKruskalTauB(
            intersection_type='linkage',
            alphabet=64,
            tokenizer=QGrams(qval=range(2, 4), skip=1),
        ).sim('gilled', 'adhering') == pytest.approx(abs=1e-7, expected=0.08653625551656562)

def test_goodman_kruskal_tau_b_dist():
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.5391997715)
    ...
    assert cmp_no_d.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.7812587413)
