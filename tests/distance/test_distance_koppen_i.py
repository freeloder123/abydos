...
def test_koppen_i_sim():
    """Test abydos.distance.KoppenI.sim."""
    # Base cases
    assert cmp.sim('', '') == 1.0
    assert cmp.sim('a', '') == 0.49936143039591324
    assert cmp.sim('', 'a') == 0.49936143039591324
    assert cmp.sim('abc', '') == 0.4987212276214834
    assert cmp.sim('', 'abc') == 0.4987212276214834
    assert cmp.sim('abc', 'abc') == 1.0
    assert cmp.sim('abcd', 'efgh') == 0.49679075738125805

    assert cmp.sim('Nigel', 'Niall') == pytest.approx(abs=1e-7, expected=0.7471079692)
    assert cmp.sim('Niall', 'Nigel') == pytest.approx(abs=1e-7, expected=0.7471079692)
    assert cmp.sim('Colin', 'Coiln') == pytest.approx(abs=1e-7, expected=0.7471079692)
    assert cmp.sim('Coiln', 'Colin') == pytest.approx(abs=1e-7, expected=0.7471079692)
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.8295625943)
...
def test_koppen_i_dist():
    """Test abydos.distance.KoppenI.dist."""
    # Base cases
    assert cmp.dist('', '') == 0.0
    assert cmp.dist('a', '') == 0.5006385696040867
    assert cmp.dist('', 'a') == 0.5006385696040867
    assert cmp.dist('abc', '') == 0.5012787723785166
    assert cmp.dist('', 'abc') == 0.5012787723785166
    assert cmp.dist('abc', 'abc') == 0.0
    assert cmp.dist('abcd', 'efgh') == 0.503209242618742

    assert cmp.dist('Nigel', 'Niall') == pytest.approx(abs=1e-7, expected=0.2528920308)
    assert cmp.dist('Niall', 'Nigel') == pytest.approx(abs=1e-7, expected=0.2528920308)
    assert cmp.dist('Colin', 'Coiln') == pytest.approx(abs=1e-7, expected=0.2528920308)
    assert cmp.dist('Coiln', 'Colin') == pytest.approx(abs=1e-7, expected=0.2528920308)
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.1704374057)
...
def test_koppen_i_corr():
    """Test abydos.distance.KoppenI.corr."""
    # Base cases
    assert cmp.corr('', '') == 1.0
    assert cmp.corr('a', '') == -0.0012771392081735637
    assert cmp.corr('', 'a') == -0.0012771392081735637
    assert cmp.corr('abc', '') == -0.002557544757033164
    assert cmp.corr('', 'abc') == -0.002557544757033164
    assert cmp.corr('abc', 'abc') == 1.0
    assert cmp.corr('abcd', 'efgh') == -0.006418485237483896

    assert cmp.corr('Nigel', 'Niall') == pytest.approx(abs=1e-7, expected=0.4942159383)
    assert cmp.corr('Niall', 'Nigel') == pytest.approx(abs=1e-7, expected=0.4942159383)
    assert cmp.corr('Colin', 'Coiln') == pytest.approx(abs=1e-7, expected=0.4942159383)
    assert cmp.corr('Coiln', 'Colin') == pytest.approx(abs=1e-7, expected=0.4942159383)
    assert cmp.corr('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.6591251885)
