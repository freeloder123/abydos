...
def test_marking_dist():
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.2)

def test_marking_sim():
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.8)

def test_marking_dist_abs():
    ...
    # Examples from paper
    assert cmp.dist_abs('cbaabdcb', 'abcba') == 2
    assert cmp.dist_abs('abba', 'a') == 2
    assert cmp.dist_abs('baab', 'a') == 3
    # The following are from the example on p. 196 of the paper, but are
    # there given in reverse order.
    assert cmp.dist_abs('ab', 'a') == 1
    assert cmp.dist_abs('abcabcabcab', 'ab') == 3
    assert cmp.dist_abs('abcabcabcab', 'a') == 7
