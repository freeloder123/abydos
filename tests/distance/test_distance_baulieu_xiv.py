...
"""abydos.tests.distance.test_distance_baulieu_xiv.

This module contains unit tests for abydos.distance.BaulieuXIV
"""


import pytest

from abydos.distance import BaulieuXIV


cmp = BaulieuXIV()

cmp_no_d = BaulieuXIV(alphabet=0)


def test_baulieu_xiv_dist():
    """Test abydos.distance.BaulieuXIV.dist."""
    # Base cases
    assert cmp.dist('', '') == 0.0
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.5882352941)

def test_baulieu_xiv_sim():
    """Test abydos.distance.BaulieuXIV.sim."""
    # Base cases
    assert cmp.sim('', '') == 1.0
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.4117647059)
