...
def test_hamann_sim():
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.9910714286)
...
def test_hamann_dist():
    """Test abydos.distance.Hamann.dist."""
    # Base cases
    assert cmp.dist('', '') == 0.0
    assert cmp.dist('a', '') == 0.0025510204081632404
    assert cmp.dist('', 'a') == 0.0025510204081632404
    assert cmp.dist('abc', '') == 0.005102040816326481
    assert cmp.dist('', 'abc') == 0.005102040816326481
    assert cmp.dist('abc', 'abc') == 0.0
    assert cmp.dist('abcd', 'efgh') == 0.012755102040816313
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.0089285714)
...
def test_hamann_corr():
    """Test abydos.distance.Hamann.corr."""
    # Base cases
    assert cmp.corr('', '') == 1.0
    assert cmp.corr('a', '') == 0.9948979591836735
    assert cmp.corr('', 'a') == 0.9948979591836735
    assert cmp.corr('abc', '') == 0.9897959183673469
    assert cmp.corr('', 'abc') == 0.9897959183673469
    ...
    assert cmp.corr('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.9821428571)
