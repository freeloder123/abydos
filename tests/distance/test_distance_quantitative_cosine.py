...
"""abydos.tests.distance.test_distance_quantitative_cosine.
...
from abydos.distance import QuantitativeCosine


cmp = QuantitativeCosine()


def test_quantitative_cosine_sim():
    """Test abydos.distance.QuantitativeCosine.sim."""
    # Base cases
    assert cmp.sim('', '') == 1.0
    ...
    assert cmp.sim('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.6674238125)

def test_quantitative_cosine_dist():
    """Test abydos.distance.QuantitativeCosine.dist."""
    # Base cases
    assert cmp.dist('', '') == 0.0
    ...
    assert cmp.dist('ATCAACGAGT', 'AACGATTAG') == pytest.approx(abs=1e-7, expected=0.3325761875)
