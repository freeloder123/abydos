...
class TestNLTKTokenizer:
    ...
    def test_nltk_tokenizer(self):
        ...
        assert sorted(tok.tokenize('').get_list()) == []
        assert sorted(tok.tokenize('a').get_list()) == ['a']

        assert sorted(tok.tokenize('NELSON').get_list()) == sorted(['NELSON'])
        assert sorted(tok.tokenize('NEILSEN').get_list()) == sorted(['NEILSEN'])
        ...
        assert (
            ...
            == sorted( [ 'Big', 'night', 'of', 'basketball', '-', '@Warriors', 'chasing', '73', 'and', 'a', 'farewell', 'for', 'an', 'all-timer', ',', '@KobeBryant', '.', 'NBA', 'fans', 'feeling', 'like', ':', ] )
        )
        ...
        assert (
            ...
            == sorted( [ 'Einstein', 'was', 'right', '!', 'Congrats', 'to', '@NSF', 'and', '@LIGO', 'on', 'detecting', 'gravitational', 'waves', '-', 'a', 'huge', 'breakthrough', 'in', 'how', 'we', 'understand', 'the', 'universe', '.', ] )
        )
