...
def test_qskipgrams():
    ...
    assert sorted(QSkipgrams().tokenize('').get_list()) == []
    assert sorted(QSkipgrams(start_stop='').tokenize('a').get_list()) == []
    assert sorted(QSkipgrams().tokenize('a').get_list()) == ['$#', '$a', 'a#']
    assert (
    ...
    assert (
        ...
        == sorted( [ '$N', '$E', '$L', '$S', '$O', '$N', '$#', 'NE', 'NL', 'NS', 'NO', 'NN', 'N#', 'EL', 'ES', 'EO', 'EN', 'E#', 'LS', 'LO', 'LN', 'L#', 'SO', 'SN', 'S#', 'ON', 'O#', 'N#', ] )
    )
    assert (
        sorted(QSkipgrams().tokenize('NEILSEN').get_list())
        == sorted( [ '$N', '$E', '$I', '$L', '$S', '$E', '$N', '$#', 'NE', 'NI', 'NL', 'NS', 'NE', 'NN', 'N#', 'EI', 'EL', 'ES', 'EE', 'EN', 'E#', 'IL', 'IS', 'IE', 'IN', 'I#', 'LS', 'LE', 'LN', 'L#', 'SE', 'SN', 'S#', 'EN', 'E#', 'N#', ] )
    )
    ...
    for key in gold_counter.keys():
        assert gold_counter[key] == pytest.approx(abs=1e-7, expected=test_counter[key])
    ...
    assert (
        QSkipgrams(qval=(2, 3), scaler='length') .tokenize('NIALL') .get_counter()
        == Counter( { '$N': 2, '$I': 2, '$A': 2, '$L': 4, '$#': 2, 'NI': 2, 'NA': 2, 'NL': 4, 'N#': 2, 'IA': 2, 'IL': 4, 'I#': 2, 'AL': 4, 'A#': 2, 'LL': 2, 'L#': 4, '$$N': 3, '$$I': 3, '$$A': 3, '$$L': 6, '$$#': 6, '$NI': 6, '$NA': 6, '$NL': 12, '$N#': 12, '$IA': 6, '$IL': 12, '$I#': 12, '$AL': 12, '$A#': 12, '$LL': 6, '$L#': 24, '$##': 6, 'NIA': 3, 'NIL': 6, 'NI#': 6, 'NAL': 6, 'NA#': 6, 'NLL': 3, 'NL#': 12, 'N##': 3, 'IAL': 6, 'IA#': 6, 'ILL': 3, 'IL#': 12, 'I##': 3, 'ALL': 3, 'AL#': 12, 'A##': 3, 'LL#': 6, 'L##': 6, } )
    )
    test_counter = (
