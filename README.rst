...
The maintained runtime targets Python 3.11-3.14. NLTK is part of the default
test matrix. SyllabiPy, PyLZSS, and paq remain optional integrations, but they
are no longer required for the core package or the default CI run because
their upstream release cadence does not consistently track the latest CPython
release.
...
To install Abydos (latest release) from PyPI using pip::
   ...
   python -m pip install abydos

To install the maintained test toolchain from a checkout::
...
It should run on Python 3.11-3.14.
...
The maintained tox environments are ``py311``, ``py312``, ``py313``,
``py314``, ``lint``, and ``docs``. So if you only want to generate
documentation, just call::

    tox -e docs

In order to only run the lint checks, call::
